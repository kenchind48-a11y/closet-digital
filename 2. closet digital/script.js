...
function attachBackButton(modalElement, closeFn) {
  ...
  var buttonGroup = header.querySelector('.button-group');
  if (!buttonGroup) {
    buttonGroup = document.createElement('div');
    buttonGroup.className = 'button-group';

    var closeBtn = header.querySelector('.icon-button');
    if (closeBtn) {
  ...
  var backButton = createBackButton('← Volver', closeFn);
...
function renderCategoryFilter() {
  ...
  label.textContent = 'Filtrar categoría';
...
function renderCategories() {
  ...
  var filter = state.filterCategory || 'Todas';
  var categoriesToShow = CATEGORY_ORDER.filter(function (category) {
  ...
  refs.categoryScroller.innerHTML = categoriesToShow
    .map(function (category) {
      ...
      var cards = items.length
        ...
        : '<div class="category-empty">Aún no hay prendas en ' + category + '.</div>';
...
function renderOutfits() {
  ...
  if (!Array.isArray(state.outfits) || state.outfits.length === 0) {
    refs.outfitList.innerHTML = '<div class="empty-state">No hay outfits guardados todavía.</div>';
    return;
  }

  refs.outfitList.innerHTML = state.outfits
    .map(function (outfit) {
      var selectedItems = Array.isArray(outfit.itemIds)
        ? outfit.itemIds
            .map(function (itemId) {
      ...
      var previewItems = selectedItems.map(function (item) {
        if (item.image) {
          return (
      ...
      var previewHtml = previewItems.length > 0
        ? previewItems.join('')
...
function getOrCreateOutfitPreviewModal() {
  ...
  var existingModal = document.querySelector('#outfitPreviewModal');
  if (existingModal instanceof HTMLElement) {
    refs.outfitPreviewModal = existingModal;
    refs.closeOutfitPreviewModal = existingModal.querySelector('.close-outfit-preview-button');
...
function renderOutfitPreviewModal(outfit) {
  ...
  var groups = getOutfitPreviewGroups(selectedItems);
...
function unlockBodyScroll() {
  ...
  var scrollY = parseInt(document.body.dataset.lockScrollY || '0', 10) || 0;
...
function openOutfitPreviewModal(outfitId) {
  ...
  var outfit = state.outfits.find(function (entry) {
  ...
  var modal = getOrCreateOutfitPreviewModal();
...
function closeOutfitModal() {
  ...
  var checkboxes = refs.outfitSelectionGrid?.querySelectorAll('input[name="outfitItem"]');
  if (checkboxes) {
...
function handleSaveOutfit() {
  ...
  var outfitName = refs.outfitName.value.trim() || 'Nuevo outfit';
  var selectedIds = Array.from(refs.outfitSelectionGrid.querySelectorAll('input[name="outfitItem"]'))
...
function handleOutfitListClick(event) {
  ...
  var deleteOutfitBtn = target.closest('[data-action="delete-outfit"]');
  if (deleteOutfitBtn) {
    return;
  }

  var outfitCard = target.closest('.outfit-card');
  if (outfitCard) {
    var outfitId = outfitCard.getAttribute('data-id');
    if (outfitId) {
...
function handleDocumentClick(event) {
  ...
  var deleteOutfitBtn = target.closest('[data-action="delete-outfit"]');
  if (deleteOutfitBtn) {
    var outfitId = deleteOutfitBtn.getAttribute('data-id');
    if (outfitId) {
